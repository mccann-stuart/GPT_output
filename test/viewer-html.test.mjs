...
test('iPhone and desktop viewers import initViewer from viewer-shared.mjs, which contains the R2 logic', () => {
  ...
  assert.match(desktopHtml, /import\s+.*initViewer.*\s+from\s+['"]\.\/viewer-shared\.mjs['"]/);
  assert.match(iphoneHtml, /import\s+.*initViewer.*\s+from\s+['"]\.\/viewer-shared\.mjs['"]/);
  ...
  for (const expected of [
    ...
    assert.match(sharedJs, new RegExp(expected.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')));
  }
});
