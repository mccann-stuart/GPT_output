...
export default function MortgageRatePredictor() {
  ...
  const current = useMemo(() => {
    return LEVERS.reduce((sum, l) => sum + (isFiveYear ? l.base5y : l.base2y), 0);
  }, [isFiveYear]);

  // Predicted total at chosen horizon = current + sum of signed bps moves
  const predicted = useMemo(() => {
    const move = LEVERS.reduce((s, l) => s + l.trends[horizon].bps, 0) / 100;
    return current + move;
  }, [current, horizon]);
    ...
    <div style={{ background: PALETTE.paper, minHeight: "100vh", color: PALETTE.ink, fontFamily: "'Inter', system-ui, sans-serif" }}>
      ...
      <div style={{ maxWidth: 920, margin: "0 auto", padding: "clamp(24px,5vw,56px) clamp(18px,4vw,32px)" }}>
        ...
        <header style={{ borderBottom: `2px solid ${PALETTE.ink}`, paddingBottom: 22, marginBottom: 28 }}>
          <div className="mono" style={{ fontSize: 11, letterSpacing: "0.18em", textTransform: "uppercase", color: PALETTE.brass, marginBottom: 12 }}>
            UK Mortgage Rate Predictor · data-linked scenario model
          </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              <div style={{ background: "rgba(176, 138, 79, 0.1)", borderLeft: `3px solid ${PALETTE.brass}`, padding: "12px 14px", borderRadius: 2 }}>
                ...
                <ul style={{ fontSize: 12.5, lineHeight: 1.45, color: PALETTE.ink, paddingLeft: 18, marginTop: 6, marginBottom: 0 }}>
                  ...
                  <li><strong>Fee Minimization:</strong> Upfront fees disproportionately raise <abbr title="Annual Percentage Rate of Charge: The average cost of the mortgage per year over its full term, factoring in the headline rate, fees, and the SVR revert rate.">APRCs</abbr> on smaller loan sizes.</li>
                </ul>
              </div>
            </div>
        ...
        <div style={{ marginTop: 32 }}>
          <IndicatorBoard indicators={LIVE_INDICATORS} />
        </div>
        ...
        <p style={{ fontSize: 12, color: PALETTE.slate, lineHeight: 1.5, marginTop: 18 }}>
          ...
          The links point to live or regularly updated sources, but this component does not fetch data automatically.
...
function IndicatorBoard({ indicators }) {
  return (
...
function IndicatorCard({ indicator }) {
  return (
    <article style={{ background: PALETTE.card, border: `1px solid ${PALETTE.line}`, borderRadius: 2, padding: "12px 13px 13px", minHeight: 174 }}>
      <div className="mono" style={{ fontSize: 10, letterSpacing: "0.08em", textTransform: "uppercase", color: PALETTE.brass, marginBottom: 6 }}>{indicator.source}</div>
      ...
      <p style={{ fontSize: 11.5, lineHeight: 1.4, color: PALETTE.slate, margin: "9px 0 0" }}>{indicator.caveat}</p>
    </article>
  );
}
