...
export async function initViewer(options = {}) {
    ...
    function isAllowedManifestFile(value) {
        return typeof value === 'string' && /^[A-Za-z0-9][A-Za-z0-9._-]*\.jsx$/.test(value);
    }
    ...
    function getSelectLabelMaxLength() {
        ...
        const fontSize = Number.parseFloat(window.getComputedStyle(selectEl).fontSize) || (isMobile ? 16 : 14);
        const usableWidth = Math.max(80, rect.width - selectLabelHorizontalChrome);
        const estimatedLength = Math.floor(usableWidth / (fontSize * selectLabelAverageCharRatio));
        return Math.max(selectLabelMinLength, Math.min(selectLabelMaxLength, estimatedLength));
    }
    ...
    function displaySelectLabelForFile(filename, source, maxLength = selectLabelMaxLength) {
    ...
    function syncSelectedSelectTitle() {
        ...
        selectEl.title = selectedOption?.dataset.fullLabel || selectedOption?.textContent || '';
    }
    ...
    function refreshSelectOptionLabels() {
        ...
        Array.from(selectEl.options).forEach((option) => {
            ...
            option.textContent = displaySelectLabelForFile(option.value, source, maxLength);
            option.title = option.dataset.fullLabel || fullSelectLabelForFile(option.value, source);
        });
    ...
    async function fetchUploadedManifest() {
        try {
            ...
            if (!res.ok) {
                console.warn(`Failed to load uploaded manifest: ${res.status} ${res.statusText}`);
                return [];
            }
            const manifest = await res.json();
            return Array.isArray(manifest.files) ? manifest.files.filter(isAllowedManifestFile) : [];
        } catch (err) {
    ...
    async function fetchManifest() {
        ...
        if (selectEl) {
            ...
            ALLOWED_FILES.forEach(f => {
                ...
                opt.textContent = displaySelectLabelForFile(f, source, getSelectLabelMaxLength());
    ...
    function getStateFromUrl(urlLike) {
        ...
        const initialFile = ALLOWED_FILES.includes(requestedFile) ? requestedFile : defaultFile;
        const initialOverrides = initialFile === sharedState?.file
    ...
    function updateFileInputs(filename) {
        if (!selectEl) return;
        const hasOption = Array.from(selectEl.options).some((opt) => opt.value === filename);
        if (hasOption) {
    ...
    function buildShareUrl() {
        ...
        if (overrides && Object.keys(overrides).length > 0) {
            url.searchParams.set(SHARE_PARAM, encodeBase64Url({
                file: currentFile,
                overrides
            }));
        } else {
    ...
    function syncPageBackgroundToComponent() {
        requestAnimationFrame(() => {
            requestAnimationFrame(() => {
                ...
                document.documentElement.style.setProperty('--viewer-bg', nextBackground);
            });
        });
    }
    ...
    async function loadJSX(filename, { initialSettingsOverrides = {}, pushHistory = true } = {}) {
        try {
            if (!ALLOWED_FILES.includes(filename)) {
                throw new Error(`Unauthorized file: ${filename}`);
            }

            root.render(React.createElement('div', { className: 'loading' }, `Loading ${filename}...`));
            ...
            transpiled = transpiled.replace(/(import|export|from)\s+['"](\.[^'"]+)['"]/g, (match, p1, p2) => {
                const absUrl = new URL(p2, fileUrl).href;
                return `${p1} '${absUrl}'`;
            });
            transpiled = transpiled.replace(/import\s*\(\s*['"](\.[^'"]+)['"]\s*\)/g, (match, p1) => {
            ...
            const defaults = isPlainObject(module.DEFAULT_SETTINGS) ? module.DEFAULT_SETTINGS : {};
            currentDefaultSettings = deepClone(defaults);
            currentInitialSettings = applyOverrides(currentDefaultSettings, initialSettingsOverrides);
    ...
    if (copyLinkBtn) {
        copyLinkBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                console.warn('Clipboard API unavailable, falling back to text selection.', err);
                if (shareUrlEl) {
                    shareUrlEl.focus();
                    shareUrlEl.select();
    ...
    window.addEventListener('popstate', () => {
        const { initialFile, initialOverrides } = getStateFromUrl(window.location.href);
        loadJSX(initialFile, {
    ...
    if (!isMobile) {
        ...
        if (minimizeBtn && topNav && bottomTab && scrollContainer && panelToggleBtn) {
            minimizeBtn.addEventListener('click', () => {
    ...
    function isSafeUploadFileName(value) {
        return typeof value === 'string' && /^[A-Za-z0-9][A-Za-z0-9._-]*\.(jsx|mjs)$/.test(value);
    }
    ...
    function setSelectedUploadFiles(files) {
        ...
        if (uploadStatusEl) {
            uploadStatusEl.textContent = selectedUploadFiles.length > 0
    ...
    try {
        ...
        root.render(React.createElement('div', { className: 'error-message' },
